...
public class NaughtyOrNice {
	private static final String PUZZLE_INPUT_PATH = "resources/day5/input.txt";

	public static void main(String[] args) {
		int numberOfGoodStrings = findNumberOfGoodStrings(
				InputReader.getFileInputAsList(PUZZLE_INPUT_PATH),
				new AlgoOne());

		System.out
				.println(String
						.format("Number of good strings in this years list with first algorithm : %d",
								numberOfGoodStrings));

		numberOfGoodStrings = findNumberOfGoodStrings(
				InputReader.getFileInputAsList(PUZZLE_INPUT_PATH),
				new AlgoTwo());

		System.out
				.println(String
						.format("Number of good strings in this years list with second algorithm : %d",
								numberOfGoodStrings));

	}

	public static int findNumberOfGoodStrings(List<String> input,
			NaughtyOrNiceAlgo algo) {
		int goodStrings = 0;

		for (String value : input) {
			goodStrings += algo.determineIfNaughtyOrNice(value);
		}
